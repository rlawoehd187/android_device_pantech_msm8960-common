...
public class SkyHLRIL extends RIL implements CommandsInterface {
    ...
    responseToStringHL(int request) {
        switch(request) {
    ...
    processUnsolicited (Parcel p, int type) {
        ...
        switch(response) {
            ...
            case RIL_UNSOL_RESPONSE_DATA_NETWORK_STATE_CHANGED:
                if (RILJ_LOGD) {
                    riljLog("[UNSL]< " + responseToStringHL(response) + " "
    ...
    public void getRadioCapability(Message response) {
        ...
        if (RILJ_LOGD) riljLog("SkyHLRIL: returning static radio capability");
    ...
    send(RILRequest rr) {
        switch (rr.mRequest) {
            ...
            case RIL_REQUEST_SET_DATA_PROFILE:
                if (RILJ_LOGD) {
                    riljLog("SkyHLRIL: received unsupported request "
                            + rr.mRequest);
                }
